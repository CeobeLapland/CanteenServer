...
public interface FoodRepository extends JpaRepository<Food, Long> {
    ...
    @Query("SELECT f FROM Food f " +
            ...
            "AND (:campusName IS NULL OR f.campusName = :campusName) " +
            "AND (:canteenName IS NULL OR f.canteenName = :canteenName) " +
            "AND (:floorName IS NULL OR f.floorName = :floorName) " +
    ...
    Page<Food> filterFoods(@Param("name") String name,
                           @Param("campusName") String campus,
                           @Param("canteenName") String canteen,
                           @Param("floorName") String floor,
     ...
     * 但是现在Food里用的是List<FoodTag> foodTags，需要改一下
     * */
    @Query("SELECT DISTINCT f FROM Food f " +
            ...
            "AND (:minPrice IS NULL OR f.price >= :minPrice) " +
            "AND (:maxPrice IS NULL OR f.price <= :maxPrice) " +
            //这里Food里只有foodTags，foodTags里只有foodId和tagId，所以下面要怎么写？
    ...
    /** 根据LocalDateTime筛选增量更新的菜品列表 */
    @Query("SELECT f FROM Food f WHERE f.updatedAt > :since")
    java.util.List<Food> findUpdatedSince(@Param("since") java.time.LocalDateTime since);
}